...
const MemorialList: React.FC<{ onSelectProfile: () => void }> = ({ onSelectProfile }) => {
  const profiles = [
    ...
    { id: '3', name: 'Ahmed Hassan', dates: '1930 - 2024', image: 'mosque' },
  ];
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {profiles.map(profile => (
            <div key={profile.id} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-stone-200 hover:shadow-xl transition-all group cursor-pointer">
              ...
              <div className="p-6">
                <h3 className="font-bold text-xl text-stone-900">{profile.name}</h3>
                <p className="text-stone-400 text-xs font-black uppercase tracking-widest mt-1">{profile.dates}</p>
              </div>
            </div>
          ))}
        </div>
