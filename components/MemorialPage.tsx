...
const MemorialPage: React.FC<{ profile: any }> = ({ profile }) => {
  ...
  if (!profile) return <div className="p-20 text-center">Loading legacy data...</div>;
            ...
            <label className="md:col-span-2 md:row-span-2 rounded-xl overflow-hidden shadow-lg bg-stone-100 cursor-pointer relative group">
              ...
              {heroImages.main ? (
                <div className="relative w-full h-full">
                  ...
                  <button onClick={(e) => removeImage(e, 'main')} className="absolute top-4 right-4 w-10 h-10 bg-red-600/90 hover:bg-red-700 text-white rounded-full flex items-center justify-center shadow-xl z-30 transition-all border-2 border-white">✕</button>
                </div>
              ) : (
            ...
            {(['thumb1', 'thumb2', 'thumb3', 'thumb4'] as const).map((slot, i) => {
              ...
              return (
                <label key={slot} className="hidden md:block col-span-1 row-span-1 rounded-xl overflow-hidden shadow-md bg-stone-100 cursor-pointer relative group">
                  <input type="file" className="hidden" accept="image/*" onChange={(e) => handleImageUpload(e, slot)} />
                  {heroImages[slot] ? (
                    <div className="relative w-full h-full">
                      <img src={heroImages[slot]!} className="w-full h-full object-cover" alt={`Gallery ${i}`} />
                      <button onClick={(e) => removeImage(e, slot)} className="absolute top-2 right-2 w-7 h-7 bg-red-600/90 hover:bg-red-700 text-white rounded-full flex items-center justify-center shadow-lg z-30 transition-all border border-white"><span className="text-[10px]">✕</span></button>
                    </div>
                  ) : (
                    <PlaceholderImage type={defaultTypes[i]} className="w-full h-full" />
                  )}
                  <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity text-[10px] text-white font-bold uppercase tracking-widest">Upload</div>
                </label>
              );
            })}
          </div>
            ...
            <div className="flex flex-wrap gap-8 text-sm text-stone-600 mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-bold text-stone-900">{profile.location || "Community Member"}</p>
                </div>
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-10 items-start">
          <div className="lg:col-span-2 space-y-24">
            ...
            <section id="sadaqah" className="scroll-mt-32">
              <SadaqahTab profile={profile} />
            </section>
            
            <section id="obituary" className="scroll-mt-32">
              <ObituaryTab profile={profile} />
            </section>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[120] p-4 backdrop-blur-md" onClick={() => setShowShareModal(false)}>
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl animate-fade-in-up" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <button className="w-full flex items-center gap-4 p-4 border border-stone-200 rounded-xl hover:bg-stone-50 transition-all font-bold text-stone-700">📧 Email</button>
              <button className="w-full flex items-center gap-4 p-4 border border-stone-200 rounded-xl hover:bg-stone-50 transition-all font-bold text-stone-700">💬 WhatsApp</button>
              <button className="w-full flex items-center gap-4 p-4 border border-stone-200 rounded-xl hover:bg-stone-50 transition-all font-bold text-stone-700">🔗 Copy Direct Link</button>
            </div>
            <button onClick={() => setShowShareModal(false)} className="mt-8 w-full py-4 bg-stone-100 text-stone-800 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-stone-200 transition-all">Close</button>
          </div>
        </div>
      )}
    </div>
...
const AboutTab: React.FC<{ profile: any }> = ({ profile }) => (
  <div className="space-y-8">
    <div className="bg-gradient-to-br from-teal-900 to-teal-800 border border-teal-700 rounded-3xl p-10 text-center shadow-2xl relative overflow-hidden">
      ...
      <p className="text-3xl text-amber-200 mb-4 leading-relaxed relative z-10" style={{fontFamily: 'serif'}}>إِنَّا لِلَّٰهِ وَإِنَّا إِلَيْهِ رَاجِعُونَ</p>
    ...
    <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-10">
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm font-black text-stone-400 uppercase tracking-widest">From The Family of {profile.full_name}</p>
        </div>
      </div>
      <p className="text-stone-700 leading-relaxed text-xl font-serif italic mb-6">
        "{profile.bio || "Alhamdulillah for the blessed years Allah (SWT) granted us."}"
      </p>
    </div>

    <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-10">
      <h3 className="font-bold text-xl mb-8 flex items-center gap-3"><span className="bg-teal-50 p-2 rounded-lg">💭</span> Spark a Memory</h3>
      <div className="grid sm:grid-cols-2 gap-4">
        {['Favorite recitation?', 'Islamic advice?', 'Character trait?', 'A story?'].map((q, i) => (
...
const MemoriesTab: React.FC = () => (
  <div className="space-y-8">
    <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-10">
      ...
      <textarea placeholder="Share a memory..." className="w-full p-6 bg-stone-50 border border-stone-200 rounded-2xl outline-none text-lg" rows={5} />
      <div className="flex gap-4 mt-6">
        ...
        <button className="px-10 py-4 border-2 border-stone-200 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-stone-100 transition-all">📷 Photo</button>
      </div>
    </div>
  </div>
);

const SadaqahTab: React.FC<{ profile: any }> = ({ profile }) => (
  <div className="space-y-8">
    <div className="bg-white rounded-[2.5rem] border border-stone-200 shadow-sm p-8 md:p-12 text-center relative overflow-hidden">
      <div className="w-20 h-20 bg-amber-100 rounded-3xl flex items-center justify-center text-4xl mx-auto mb-8 shadow-inner">🕌</div>
      <h3 className="font-serif text-3xl text-stone-900 mb-4">Orphan Education & Care</h3>
      <p className="text-stone-600 leading-relaxed mb-10 text-lg">Support this project in the name of {profile.full_name}.</p>
      <div className="bg-stone-50 rounded-3xl p-8 border border-stone-100 mb-10">
        <a href={profile.charity_url || "https://www.launchgood.com"} target="_blank" rel="noopener noreferrer" className="block w-full bg-teal-800 text-white py-5 rounded-2xl font-black text-xl uppercase tracking-widest hover:bg-teal-900 shadow-xl no-underline">Confirm & Donate →</a>
      </div>
    </div>
  </div>
);

const ObituaryTab: React.FC<{ profile: any }> = ({ profile }) => (
  <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-12">
    ...
    <div className="prose prose-stone max-w-none space-y-8">
      <p className="text-xl leading-relaxed text-stone-700 font-medium"><strong className="text-teal-900">{profile.full_name}</strong>, returned to his Lord on {profile.death_date}.</p>
      <p className="text-lg leading-relaxed text-stone-600">{profile.obituary_text || "A full obituary will be shared soon."}</p>
    </div>
  </div>
);
...
const EventsTab: React.FC<any> = ({ rsvpOpen, setRsvpOpen }) => {
  const events = [
    { id: 1, title: 'Janazah Prayer', date: 'Upcoming', time: 'TBD', location: 'Local Masjid', icon: '🕌' },
  ];
  return (
    <div className="space-y-8">
      <h2 className="text-3xl font-black mb-10 uppercase tracking-tight">Services</h2>
      {events.map(event => (
        <div key={event.id} className="bg-white rounded-3xl border border-stone-200 shadow-sm p-10 flex flex-col md:flex-row gap-8 items-center md:items-start group hover:border-teal-800 transition-all">
          <div className="w-20 h-20 rounded-2xl bg-teal-50 flex items-center justify-center text-4xl shadow-inner">{event.icon}</div>
          <div className="flex-1 space-y-4">
            <h3 className="font-black text-2xl uppercase tracking-tighter">{event.title}</h3>
            <div className="grid sm:grid-cols-2 gap-4 font-bold text-stone-800"><p>📅 {event.date}</p><p>🕐 {event.time}</p><p className="sm:col-span-2">📍 {event.location}</p></div>
          </div>
        </div>
      ))}
    </div>
...
const TimelineTab: React.FC = () => (
  <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-12 text-center"><h2 className="text-3xl font-black mb-12 uppercase">Legacy Timeline</h2><p className="text-stone-400">Timeline data coming soon.</p></div>
);

const FavoritesTab: React.FC = () => (
  <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-12 text-center"><h2 className="text-3xl font-black mb-10 uppercase">Spiritual Affinities</h2><p className="text-stone-400">Spiritual data coming soon.</p></div>
);

const Sidebar: React.FC<{ profile: any }> = ({ profile }) => (
  <div className="space-y-8">
    <div className="bg-teal-900 rounded-[2rem] p-10 text-white shadow-2xl relative overflow-hidden">
      <div className="absolute inset-0 bg-pattern opacity-10" />
      <div className="flex items-center gap-4 mb-6 relative z-10"><div className="w-16 h-16 rounded-2xl bg-white/10 flex items-center justify-center text-3xl">💝</div><h3 className="font-black text-2xl uppercase tracking-tighter">Sadaqah Jariyah</h3></div>
      <p className="text-teal-100 text-lg font-medium mb-8 leading-relaxed relative z-10">Honor his legacy through everlasting charity.</p>
      <a href={profile.charity_url || "#"} target="_blank" className="block text-center w-full bg-amber-500 text-teal-950 py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-amber-400 no-underline relative z-10">Give Sadaqah Jariyah →</a>
    </div>

    <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-8">
      <h3 className="font-black text-xl mb-8 uppercase tracking-tight">Survived By</h3>
      <div className="space-y-6">
        {profile.survivors_json && profile.survivors_json.length > 0 ? (
          profile.survivors_json.map((p: any, i: number) => (
            <div key={i} className="flex items-center gap-4 group cursor-default">
              <div className="w-14 h-14 rounded-2xl bg-stone-50 text-teal-800 flex items-center justify-center font-black text-xl shadow-inner">{p.name[0]}</div>
              <div><p className="font-black text-stone-900 uppercase tracking-tight text-sm">{p.name}</p><p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">{p.relation}</p></div>
            </div>
          ))
        ) : (
          <p className="text-stone-400 text-sm">No survivors listed yet.</p>
        )}
      </div>
    </div>
  </div>
